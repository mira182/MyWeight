...
public class MuscleQualityLineChart extends AbstractLineChart {

    public MuscleQualityLineChart(Context context, View view, List<WeightReportDTO> weightReport) {
    ...
    List<Entry> createEntries(List<WeightReportDTO> weightReport) {
        ...
        for (int i = 1; i < weightReport.size(); i++) {
            ...
            entries.add(new Entry(x_points, y_points, ChartType.MUSCLE_QUALITY.getChartName()));
        }
        return entries;
    }
