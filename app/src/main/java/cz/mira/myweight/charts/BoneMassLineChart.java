...
public class BoneMassLineChart extends AbstractLineChart {

    public BoneMassLineChart(Context context, View view, List<WeightReportDTO> weightReport) {
    ...
    List<Entry> createEntries(List<WeightReportDTO> weightReport) {
        ...
        for (int i = 1; i < weightReport.size(); i++) {
            ...
            entries.add(new Entry(x_points, y_points, ChartType.BONE_MASS.getChartName()));
        }
        return entries;
    }
    ...
    float getXValue(List<WeightReportDTO> weightReport, int index) {
        return weightReport.get(index).getBoneMass().floatValue();
    }
