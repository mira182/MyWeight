...
public class MetabolicAgeLineChart extends AbstractLineChart {

    static final String CHART_NAME = "Metabolic age";

    public MetabolicAgeLineChart(Context context, View view, List<WeightReportDTO> weightReport) {
    ...
    List<Entry> createEntries(List<WeightReportDTO> weightReport) {
        ...
        for (int i = 1; i < weightReport.size(); i++) {
            ...
            entries.add(new Entry(x_points, y_points, ChartType.METABOLIC_AGE.getChartName()));
        }
        return entries;
    }
    ...
    void setUpLineYAxis(YAxis axis) {
        final LimitLine limit = new LimitLine(32f);
        limit.setLineWidth(1f);
        limit.enableDashedLine(10f, 10f, 0f);

        axis.removeAllLimitLines();
        axis.addLimitLine(limit);
    }
}
