...
public class BmiLineChart extends AbstractLineChart {

    public BmiLineChart(Context context, View view, List<WeightReportDTO> weightReport) {
    ...
    List<Entry> createEntries(List<WeightReportDTO> weightReport) {
        ...
        for (int i = 1; i < weightReport.size(); i++) {
            ...
            entries.add(new Entry(x_points, y_points, ChartType.BMI.getChartName()));
        }
        return entries;
    }
    ...
    void setUpLineYAxis(YAxis axis) {
        final LimitLine limit = new LimitLine(25f);
